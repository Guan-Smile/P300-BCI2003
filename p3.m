...
avgdata1=[];
avgdata2=[];
trials=unique(trialnr);%% 强化次数：540次=180*3个字符
max_stimuluscode=max(StimulusCode);%%%最大行列数
stimulusdata=zeros(max_stimuluscode, triallength);
stimuluscount=zeros(max_stimuluscode);
for cur_trial=min(trials)+1:max(trials)-1  %%%cur_trial=1:540
 ...
 trialdata=signal(min(trialidx):min(trialidx)+triallength-1, :); %取出每次强调的过程中的数据
