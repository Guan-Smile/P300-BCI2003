...
%    CHAR1=strfind((t(i).PhaseInSequence(100:end)'),[2 3])
   ...
   [X_part_train,y_part_train,StimulusCode]=cut_everymat_block(t(i).signal...
       ,t(i).StimulusType,t(i).trialnr,size_CHAR1(i),t(i).StimulusCode,2,5);%%%%or01代表采集强调0还是1的点;block代表多少组取平均
...
%     [X_part_test,StimulusCode]=cut_everymat_test(t_test(j).signal,t_test(j).trialnr,size_CHAR2(j),t_test(j).StimulusCode);
% %    All_testcount=size_CHAR1(i)*cont_forchar;%%%%%%%%%实验总次数，01中，三个字母540次
