...
%    CHAR1=strfind((t(i).PhaseInSequence(100:end)'),[2 3])
   ...
   [Chang_signal]= FFT_detection(t(i).signal,240);
   
%    t(i).signal=CSP(t(i).signal);
%    FFT_detection(t(i).signal,240);
   [X_part_train,y_part_train,StimulusCode]=cut_everymat_block (Chang_signal...
...
%      [Chang_signal]= FFT_detection(t_test(j).signal,240);
%     [X_part_test,StimulusCode]=cut_everymat_test2(Chang_signal,t_test(j).trialnr,size_CHAR2(j),t_test(j).StimulusCode,BLOK);
% %    All_testcount=size_CHAR1(i)*cont_forchar;%%%%%%%%%实验总次数，01中，三个字母540次
...
% save(['E:/BCI_IIdata/data/fft/M_X_train_all_block_fft_',num2str(BLOK),'.mat'],'X_train')
% save(['E:/BCI_IIdata/data/fft/M_y_train_all_block_fft_',num2str(BLOK),'.mat'],'y_train')
% save(['E:/BCI_IIdata/data/fft/M_z_train_all_block_fft_',num2str(BLOK),'.mat'],'StimulusCode_train')

% save(['E:/BCI_IIdata/data/fft/M_X_test_all_block_fft_',num2str(BLOK),'.mat'],'X_test')%%%%%%行156，64列,540个字符
% save(['E:/BCI_IIdata/data/fft/M_z_test_all_block_fft_',num2str(BLOK),'.mat'],'StimulusCode_test')%%%%%%行156，64列,540个字符
